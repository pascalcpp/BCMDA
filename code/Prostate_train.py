...
parser.add_argument("--corr_resolution", default=96, type=int)
parser.add_argument("--temp", default=0.30, type=float)
        ...
        for i_batch in range(1, args.num_eval_iter + 1):
            ...
            with amp_cm():
                with torch.no_grad():
                    ...
                    better_mask_ulb = (prob_ulb_sim[:, 1, :, :] > threshold).float().unsqueeze(1)
                ...
                with torch.no_grad():
                    new_lb_x_w, new_ulb_x_w = generate_new_image_1c(last_fts_lb_x_w_ema,
                                                                               last_fts_ulb_x_w_ema,
                                                                               lb_x_w_raw.clone(),
                    ...
                    #
                    better_mask_mix_ulb_fix_w = (prob_mix_ulb_fix_w_sim[:, 1, :, :] > threshold).float().unsqueeze(1)
                    mix_ulb_fix_w_prob = prob_mix_ulb_fix_w_sim * better_mask_mix_ulb_fix_w + prob_mix_ulb_fix_w_linear * (
                                1 - better_mask_mix_ulb_fix_w)
