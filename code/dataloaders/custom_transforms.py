...
class Normalize_tf(object):
    ...
    def __call__(self, sample):
        ...
        if 'strong_aug' in sample.keys():
            ...
            # strong = np.clip(strong, 0, 255).astype(np.float32)
            # sample['strong_raw'] = strong.copy()
            # strong = np.clip(strong, 0.0, 255.0)
            #
            strong /= 127.5
            strong -= 1.0
            #
            ...
            #     s_min = 0.0
            #     s_max = 255.0
            # strong = (strong - s_min) / (s_max - s_min)
